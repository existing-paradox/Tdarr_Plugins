...
const details = (): IpluginDetails => ({
  name: 'Trigger Stash Scan',
  description: 'Trigger Stash Scan',
  style: {
    borderColor: 'green',
  },
  tags: '',
...
const plugin = async (args: IpluginInputArgs): Promise<IpluginOutputArgs> => {
  ...
  const requestConfig = {
    ...
    data: JSON.stringify({ query: `mutation { metadataScan(input: {paths: [${allFiles ? "" : JSON.stringify(fileDir)}]}) }` }),
  };
